...
まず機構の整理：Guardrails は入出力**テキスト**を検査・マスクする機能ですが、検査自体は AWS 内の検出サービスで行われます。したがって「データを AWS の外へ出さない」という課題には寄与せず、後述の通り本システムでは以下の 2 点が成立しません（当初挙げていた第 3 の理由「日本語検出不能」は実測で撤回、表後に補記）。
...
**補記（2026-09-18 実測による修正）**：当初は「日本語固有の PII 実体がなく検出できない」ことを第 3 の理由として挙げていたが、実測（us-east-1、BLOCK 版 guardrail）では一般実体（NAME / ADDRESS）が日本語氏名（山田太郎）・住所（東京都千代田区…）を検出したため、この理由は撤回する。日本語が検出できてしまう分、理由 2（マスクが審査業務と矛盾）はより強く当てはまる。なお同実測で、独立チェック API（ApplyGuardrail）の入力方向では ANONYMIZE 指定の PII が評価されない事象（BLOCK は同一条件で正常作動）も確認しており、検証継続中である。
...
2. **Guardrails の追加は「情報漏洩防止」の目的に対して効果がなく**（第 2 節の理由）、導入提案は不要
